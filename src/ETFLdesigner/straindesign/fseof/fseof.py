...
def simulateGrowth(model, target, c_source,c_uptake=1, alpha=1, tol=1e-6):
    """
    ...
    Returns:
    - flux: (pandas.DataFrame) a DataFrame of the optimized flux distribution
    ...
    flux = optModel(model, target,tol)
    return flux


def optModel(model, target, tol):
    """
   optimize for a given objective. Firstly,maximize the objective reaction,and then  a protein pool minimization is performed subject to the optimal
    ...
    if sol.status == 'optimal':
        ...
        # minimize enzyme usage by maxing dummy enzyme
...
def k_matrix_filter(model, k_matrix, alpha, tol):
    ...
    # 4.Replace any Inf value with 1000 (maximum value is ~700):
...
def flux_scanning(model, targetID, c_source,c_uptake, alpha, tol=1e-10, filterG=False):
    """
    ecFlux_scanning
    Args:
        model (cobra.Model): ecModel with total protein pool constraint.
    ...
    FC = {}
    FC['flux_WT'] = simulateGrowth(model=model, target=targetID, c_source=c_source,c_uptake= c_uptake,alpha=1)
    ...
    for i in range(len(alpha)):
        FC['flux_MAX'] = simulateGrowth(model=model, target=targetID, c_source=c_source,c_uptake= c_uptake,alpha=alpha[i])
...
def run_FSEOF(model, targetID, c_source,c_uptake, alphaLims, Nsteps):
    """
    ...
    # Run FSEOF analysis
    results = flux_scanning(model=model, targetID=targetID, c_source=c_source,c_uptake=c_uptake, alpha=alphaV)
