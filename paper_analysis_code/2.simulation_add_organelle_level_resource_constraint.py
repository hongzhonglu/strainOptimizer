...
from strainOptimizer.manipulation.constraint.total_resource_allocation import constrain_enzymes,add_organelleproteome_constraints
...
productParam_dict={
    '2-phenylethanol':{'productName':'2-phenylethanol',
                       ...
                       'etfl_filepath':'examples/models/yeast/cEFL_2-phenylethanol.json'},
...
for product_key,productParam in productParam_dict.items():
    ...
    for model_key,modelParams in modelParams_dict.items():
        ...
        output_file=f'{model_key}_{productName}_gluc_{c_uptake}_bounds_no_mitchondria_ecFactory_result.xlsx'
        # check if the output file already exists
        if pd.io.common.file_exists(f'analysis_code/results/organelle_proteomic_constraints/{output_file}'):
        ...
        # add main organelles proteomic constraints
        df_organelle_protein_fractions=pd.read_csv(r'analysis_code/sce_organelle_proteome_fractions.csv',index_col=0)
        # organelleList=[i for i in model.compartments.values() if i in df_organelle_protein_fractions.index]
        # df_organelle_protein_fractions=df_organelle_protein_fractions.loc[organelleList]
        # only keep max > 0.05
        ...
        model=add_organelleproteome_constraints(model,
                                                proteome_fractions=df_organelle_protein_fractions,
                                                method='bounds',
                                                # method='ub',
        ...
        with pd.ExcelWriter(f'analysis_code/results/organelle_proteomic_constraints/{output_file}') as writer:
            for key in results.keys():
