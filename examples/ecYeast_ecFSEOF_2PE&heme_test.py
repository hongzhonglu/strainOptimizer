# -*- coding: utf-8 -*-
# date : 2023/3/5 
# author : wangh
# file : ecYeast_ecFSEOF_2PE_test.py
# project : etfl
import os
import sys
import numpy as np
sys.path.append(r"D:\code\github\etfl\ETFLdesigner\ETFLdesigner")

# load packages
import pandas as pd
from ETFLdesigner.ETFLdesigner.io.ecModel import load_ecmodel
from ETFLdesigner.ETFLdesigner.strain_design.ecFactory import run_ecFactory


# load 2-PE model
# ecYeast=load_ecmodel('code_etfl/ETFLdesigner/models/ecYeastGEM_batch.xml')
# model=ecYeast
# product_name='2-phenylethanol'
# product_id='r_1589'    # 2-PE exchange rxn


# # heme model
heme_ecYeast=load_ecmodel('models/heme_ecYeastGEM.xml')
model=heme_ecYeast
product_name='heme_a'
product_id='EX_heme_a'
model.reactions.get_by_id(product_id).bounds=0,0


# calculate the max biomass yield
medium=model.medium
model.reactions.get_by_id('r_1714_REV').bounds=1,1
sol=model.optimize()
# set tolerance
model.tolerance=1e-9
# parameters for 2-phenylethanol production design in yefl model
c_source="r_1714_REV"      # glucose exchange rxn
growth_id='r_2111'
model.objective=growth_id# biomass rxn
c_uptake=1
gluc_MW=0.180156  # g/mmol
max_yield=sol.objective_value/(c_uptake*gluc_MW) # gDW / gGluc
expYield=max_yield*0.5
alphaLims=(0.25*max_yield,0.75*max_yield)

# prepare parameters for ecFSEOF
modelParam=pd.Series()
modelParam['targetID']=product_id
modelParam['c_source']=c_source
modelParam['c_uptake']=c_uptake
modelParam['productName']=product_name

action_thresholds=[0.05,0.3,1.05]     # rules for overexpression, knockout and knockdown
# set a timer
import time
start_time = time.time()
print('start time:',start_time)
# run ecFSEOF
results=run_ecFactory.run_ecFactory_design(model=model, modelParam=modelParam, expYield=expYield,alphaLims=alphaLims,action_thresholds=action_thresholds,remove_essential=True,model_type='ecGEM')
end_time = time.time()
print('end time:',end_time)
print('time cost:',end_time-start_time)

# print FSEOF results
print('FSEOF results:')
genetable=results['geneTable']
print('OE:',genetable[genetable['actions']=='OE'].shape[0])
print('KD:',genetable[genetable['actions']=='KD'].shape[0])
print('KO:',genetable[genetable['actions']=='KO'].shape[0])


# EUVA results
print('EUVA results:')
print('leval 1 candidates:',genetable[genetable['target_priority_leval']==1].shape[0])
print('leval 2 candidates:',genetable[genetable['target_priority_leval']==2].shape[0])
print('leval 3 candidates:',genetable[genetable['target_priority_leval']==3].shape[0])


# evaluate the minimal set
min_result=results['min_set_analysis_result']
min_set_list=min_result[min_result['score']<0.99].index.tolist()
print(min_result[min_result['score']<0.99].index.tolist())

# save results into excel file
for key in results.keys():
    if isinstance(results[key],np.ndarray):
        results[key]=pd.Series(results[key])
    # 检查是否为dict
    if isinstance(results[key],dict):
        results[key]=pd.Series(results[key])
with pd.ExcelWriter('code_etfl/ETFLdesigner/output/ecYeast_2PE_design_results.xlsx') as writer:
    for key in results.keys():
        results[key].to_excel(writer, sheet_name=key)


# load ecFactory results
ecFactory_results=pd.read_csv('code_etfl/reference/ecFactory/results/2_phenylethanol_targets/candidates_L2.txt',sep='\t',index_col=0)
ecfactory_candidates=ecFactory_results.index.tolist()
ecfactory_candidates_l3=pd.read_csv('code_etfl/reference/ecFactory/results/heme_targets/candidates_L3.txt',sep='\t',index_col=0).index.tolist()
ecfactory_candidates_l1=pd.read_csv('code_etfl/reference/ecFactory/results/heme_targets/candidates_L1.txt',sep='\t',index_col=0).index.tolist()


common_l3=set(min_set_list).intersection(set(ecfactory_candidates_l3))
comon_l1=set(min_set_list).intersection(set(ecfactory_candidates_l1))

all_candidates=results['geneTable'].index.tolist()
euvacandidates_l3=results['geneTable'][results['geneTable']['target_priority_leval'].isin([1,2,3])].index.tolist()
all_common_l3=set(all_candidates).intersection(set(ecfactory_candidates_l3))
all_common_l1=set(all_candidates).intersection(set(ecfactory_candidates_l1))
euva_common_l3=set(euvacandidates_l3).intersection(set(ecfactory_candidates_l3))
euva_common_l1=set(euvacandidates_l3).intersection(set(ecfactory_candidates_l1))